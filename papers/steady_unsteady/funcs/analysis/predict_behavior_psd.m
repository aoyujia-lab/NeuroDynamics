function S = predict_behavior_psd(all_vects, all_behav, cov, C)
...
%   C         : config
%   Kfallback : (optional) fallback K for feature selection
%               Rule (per pos/neg/com separately):
%                 - if nnz(p<thresh) >= Kfallback: use all p<thresh
%                 - else: use top-Kfallback smallest p within the candidate set
%               If Kfallback <= 0: disable fallback (threshold only)
%
% OUTPUT (struct S)
...
S.meta.note      = 'LOO CPM on ROI-level features (threshold + fallback-to-topK if needed)';
...
pos_masks = false(nROI, nSubj);
neg_masks = false(nROI, nSubj);
com_masks = false(nROI, nSubj);
...
for leftout = 1:nSubj
    ...
    Xtrain = all_vects(:, train_idx);   % [nROI x (nSubj-1)]
    ytrain = all_behav(train_idx);      % [(nSubj-1) x 1]
    nTrain = sum(train_idx);

    % ---- ROI-wise association on TRAIN ----
    switch fs_option
        case 1
            [r_vec, p_vec] = corr(Xtrain', ytrain);
        case 2
    ...
    % COM candidates: all ROI (p<thresh), fallback to topK overall if too few
    com_sig = (p_vec < thresh);
    if use_fallback && nnz(com_sig) < Kfallback
        com_mask = topk_mask_by_p(p_vec, true(nROI,1), Kfallback);
    else
        com_mask = com_sig;
    end
    ...
    % ---- sum features ----
    % pos-only / neg-only sums
    sumpos_train = sum(Xtrain(pos_mask, :), 1)'; % [nTrain x 1]
    sumneg_train = sum(Xtrain(neg_mask, :), 1)';

    sumpos_test = sum(all_vects(pos_mask, leftout));
    sumneg_test = sum(all_vects(neg_mask, leftout));
    ...
    sumposC_test = sum(all_vects(com_pos_mask, leftout));
    sumnegC_test = sum(all_vects(com_neg_mask, leftout));

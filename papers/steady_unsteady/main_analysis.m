...
for ir = 1:nRange
    ...
    % svm_stat = psdsvm_permtest(psd_range_z(ir).power_roi, C);
end
    ...
    for ic = 2
        ...
        for ib = 1:2
            ...
            for iroi = 1:360
                ...
                r_vec(iroi) = r(1,2); p_vec(iroi) = p(1,2);
            end
...
mean_psd_rest_steady_glm = mean(PSD_OUT.rest_steady,2);
mean_psd_rest_steady_glm = mean(mean_psd_rest_steady_glm,3);
...
mean_psd_rest_unsteady_glm = mean(PSD_OUT.rest_unsteady,2);
mean_psd_rest_unsteady_glm = mean(mean_psd_rest_unsteady_glm,3);
...
band = 2;
mean_psd_JR(:,1) = mean(results_unsteady.a_vel.psd_task(range{band},:),1);
...
[R2_quad, beta, y_hat] = quadratic_r2(x, y)
...
psd_alpha = zeros(size(PSD_prob_task_alpha, 3), numel(networks));  % 501 x 10
...
psd_beta = zeros(size(PSD_prob_task_beta, 3), numel(networks));    % 501 x 10
for i = 1:numel(networks)
    net = networks{i};
...
psd_r0 = zeros(size(PSD_prob_task_r0, 3), numel(networks));        % 501 x 10
for i = 1:numel(networks)
    net = networks{i};
